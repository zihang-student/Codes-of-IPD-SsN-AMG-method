function [zeta,itamg,resamg,info] = Hybrid_AMG(prob_data,amg_options)
% Original system:
%                   He * zeta = z
...
if length(sizes) == 1 % A0/H0 is connected
    ...
    [u,itamg,resamg] = Class_AMG(Ae,f,amg_options);
    
%     if resamg > amg_options.retol %%In Class 1 you need comment this part and line 73. Do not comment it in Class 4!
...
if length(sizes) > 1  % A0/H0 is disconnected
    ...
    if ~isempty(large_size)
        for k = large_size
            ...
            [dk,itamgk,resamgk] = Class_AMG(Aek,fk,amg_options);
            
            
%             if resamgk > amg_options.retol %%In Class 1 you need comment this part and line 43. Do not comment it in Class 4!
%                 amg_options.isnsp = ~amg_options.isnsp;
%                 [dk,itamgk,resamgk] = Class_AMG(Aek,fk,amg_options);
%             end
...
% % Debug the iteration number
% if itamg == amg_options.maxit
%     error('itamg == amg_options.maxit');
% end
%% zeta
